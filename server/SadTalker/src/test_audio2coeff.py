...
class Audio2Coeff():
    ...
    def __init__(self, sadtalker_path, device):
        ...
        try:
            print("Loading audio2pose checkpoint from:", sadtalker_path['audio2pose_checkpoint'])
            load_cpk(sadtalker_path['audio2pose_checkpoint'], model=self.audio2pose_model, device=device)
            print("Loaded audio2pose checkpoint successfully ✅")
        except Exception as e:
            print("Failed to load audio2pose checkpoint ❌", e)
            raise
        ...
        netG.eval()
        try:
            print("Loading audio2exp checkpoint from:", sadtalker_path['audio2exp_checkpoint'])
            load_cpk(sadtalker_path['audio2exp_checkpoint'], model=netG, device=device)
            print("Loaded audio2exp checkpoint successfully ✅")
        except Exception as e:
            print("Failed to load audio2exp checkpoint ❌", e)
            raise
